...
const ChildDashboardPage = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (paths.length) {
        const pathLessons = await listLessonsForPath(paths[0].id, activeChild.age);
        ...
        <div className="rounded-3xl bg-white p-6 shadow-lg shadow-slate-200/60">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            ...
            <Card className="bg-mint/20">
              ...
              <p className="text-2xl font-bold text-ink">{insights.streak?.current_days ?? 0} days</p>
              <p className="text-sm text-slate-600">Longest {insights.streak?.longest_days ?? 0}</p>
            </Card>
            ...
            <div className="mt-4 space-y-3">
              {lessons.map((lesson) => (
                <div key={lesson.id} className="flex items-center justify-between rounded-2xl border border-slate-100 p-4">
                  ...
                  <Button onClick={() => navigate(`/child/lesson?lessonId=${lesson.id}`)}>Start</Button>
                </div>
              ))}
            </div>
          )}
